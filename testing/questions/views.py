...
def test_questions(request: HttpRequest, name: str) -> HttpResponse:
    studentresult = models.StudentResult.objects.filter(student=request.user, test__name=name).first()
    if studentresult is not None:
        return render(request, 'questions/student_result.html',
                {'user': request.user, 'result': studentresult.result,
                'questions': studentresult.amount_questions})                                        
